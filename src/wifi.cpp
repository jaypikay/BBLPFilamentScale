...
void setupWiFi() {
  ...
  new (&custom_mqtt_host)
      WiFiManagerParameter("mqtt_host", "MQTT Server Host", "", 40);
  new (&custom_mqtt_port) WiFiManagerParameter("mqtt_port", "MQTT Server Port",
                                               "1883", 6, "type=\"number\"");
  new (&custom_mqtt_username)
      WiFiManagerParameter("mqtt_username", "MQTT Username", "", 20);
  new (&custom_mqtt_password) WiFiManagerParameter(
      "mqtt_password", "MQTT Password", "", 32, "type=\"password\"");
...
void saveConfigCallback() {
  if (!String(custom_mqtt_host.getValue()).isEmpty() &&
      strcasecmp(custom_mqtt_host.getValue(), mqtt_host.c_str()) != 0) {
  ...
  if (!String(custom_mqtt_username.getValue()).isEmpty() &&
      strcasecmp(custom_mqtt_username.getValue(), mqtt_username.c_str()) != 0) {
    debug_println("*CONF: mqtt_username has changed...");
    mqtt_username = String(custom_mqtt_username.getValue());
  }
  if (!String(custom_mqtt_password.getValue()).isEmpty() &&
      strcasecmp(custom_mqtt_password.getValue(), mqtt_password.c_str()) != 0) {
